...
## 1. JavaScript hace una cosa a la vez
...
## 2. El Event Loop: cómo JavaScript gestiona el tiempo
...
## 3. Callbacks: funciones que se llaman después
...
## 4. Promises: una forma mejor de gestionar el tiempo
...
esperar(2)
    .then(mensaje => console.log(mensaje))  // 'Han pasado 2 segundos'
    .catch(error => console.error(error.message));

esperar(-1)
    .then(mensaje => console.log(mensaje))
    .catch(error => console.error(error.message));  // 'Los segundos no pueden ser negativos'
```
...
Esto es lo que permite la cadena plana en lugar de la pirámide anidada.

---

## 5. async / await: Promises con cara humana
...
// Con async/await — exactamente lo mismo, diferente cara
...
## 6. El camino completo

Ahora que tienes el contexto completo, tiene sentido ver la evolución:
...
Los tres mecanismos siguen conviviendo en el código real. Los callbacks no han desaparecido — `addEventListener`, `forEach`, `setTimeout` los siguen usando. Las Promises aparecen constantemente en librerías y en el código que devuelve `fetch`. Y `async/await` es lo que escribes tú cuando creas código nuevo.


## 7. Para practicar antes de las APIs

Antes de pasar a `fetch`, practica con `setTimeout` para sentirte cómodo con la asincronía sin la complejidad adicional de la red.
...
Cuando los tres ejercicios funcionen y entiendas por qué cada uno funciona como funciona, estás listo para las APIs.
